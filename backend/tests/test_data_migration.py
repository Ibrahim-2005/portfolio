...
def test_data_migration(db_session):
    ...
    try:
        db_session.execute(text("ALTER TABLE projects ADD COLUMN tech_stack_legacy VARCHAR"))
    ...
    db_session.execute(
        text("INSERT INTO projects (title, description, tech_stack_legacy, tech_stack, sort_order, featured) VALUES ('Proj1', 'Desc', :tech, '[]', 1, false)"),
        {"tech": tech_legacy}
    )
    
    # 2. Insert Legacy Skills
    db_session.execute(
        text("INSERT INTO skills (name, category, proficiency_legacy, sort_order, domain_id, proficiency) VALUES ('Python', 'Backend', 'Intermediate', 1, null, 0)")
    )
    db_session.execute(
        text("INSERT INTO skills (name, category, proficiency_legacy, sort_order, domain_id, proficiency) VALUES ('HTML', 'Frontend', 'Expert', 2, null, 0)")
    )
    ...
    # --- Verify Projects ---
    proj = db_session.execute(text("SELECT tech_stack, tech_stack_legacy, featured FROM projects WHERE title = 'Proj1'")).fetchone()
    ...
    # --- Verify Skills ---
    domains = db_session.execute(text("SELECT id, name FROM skill_domains ORDER BY name")).fetchall()
    assert len(domains) == 2
    assert domains[0].name == "Backend"
    assert domains[1].name == "Frontend"
    
    s_backend = db_session.execute(text("SELECT proficiency, proficiency_legacy, category FROM skills WHERE name = 'Python'")).fetchone()
    ...
    s_frontend = db_session.execute(text("SELECT proficiency, proficiency_legacy, category FROM skills WHERE name = 'HTML'")).fetchone()
    ...
    proj2 = db_session.execute(text("SELECT tech_stack, tech_stack_legacy, featured FROM projects WHERE title = 'Proj1'")).fetchone()
...
def test_migration_unknown_proficiency(db_session):
    ...
    db_session.execute(
        text("INSERT INTO skills (name, category, proficiency_legacy, sort_order, domain_id, proficiency) VALUES ('Python', 'Backend', 'Jedi', 1, null, 0)")
    )
    db_session.commit()
