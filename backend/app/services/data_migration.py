...
PROFICIENCY_MAP = {"Beginner": 25, "Intermediate": 60, "Advanced": 80, "Expert": 95}
...
def migrate_phase3_data(session: Session) -> dict:
    ...
    skills = session.execute(
        text("SELECT id, category, proficiency_legacy FROM skills")
    ).fetchall()

    for skill_id, category, proficiency_legacy in skills:
        # Create or find domain
        domain = session.execute(
            text("SELECT id FROM skill_domains WHERE name = :name"), {"name": category}
        ).fetchone()
        ...
        session.execute(
            text(
                "UPDATE skills SET domain_id = :domain_id, proficiency = :prof WHERE id = :id"
            ),
    ...
    projects = session.execute(
        text("SELECT id, tech_stack_legacy FROM projects")
    ).fetchall()
        ...
        if tech_legacy:
            ...
            if isinstance(tech_legacy, str):
                try:
                    ...
                    tech_list = (
                        tech_legacy.strip("{}").split(",") if "{" in tech_legacy else []
                    )
            else:
            ...
            for tech in tech_list:
                ...
                if tech:
                    new_tech_stack.append({"name": tech, "icon": get_tech_icon(tech)})
