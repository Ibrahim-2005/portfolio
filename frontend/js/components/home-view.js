...
export async function renderHome() {
    ...
    const rolesHtml = config.roles && config.roles.length > 0 
        ? config.roles.map(r => `<span class="badge">${r.icon ? r.icon + ' ' : ''}${r.label}</span>`).join('\n        ')
        : '';

    const socialLinksHtml = config.social_links && config.social_links.length > 0
        ? `<div class="home-socials" style="margin-top: 1.5rem; display: flex; gap: 20px; align-items: center;">` + 
          config.social_links
            .filter(s => s.enabled)
            .sort((a,b) => a.sort_order - b.sort_order)
            .map(s => `<a href="${s.url}" target="_blank" style="color: var(--fg-muted); text-decoration: none; font-size: 1.2rem; transition: color 0.2s;" onmouseover="this.style.color='var(--fg-default)'" onmouseout="this.style.color='var(--fg-muted)'" title="${s.platform}">${s.icon || s.platform}</a>`)
            .join('') + 
          `</div>`
        : '';
