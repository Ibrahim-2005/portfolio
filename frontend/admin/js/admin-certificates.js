...
export async function initCertificatesEditor() {
  ...
  try {
    const config = await adminApi.getCertificatesConfig();
    if (config) {
      document.getElementById("certificates-content").value =
        config.content || "";
    }
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const data = {
          content:
            document.getElementById("certificates-content").value || null,
        };

        await adminApi.updateCertificatesConfig(data);
