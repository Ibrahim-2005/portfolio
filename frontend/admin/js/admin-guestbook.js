...
async function loadGuestbook() {
    ...
    try {
        ...
        if (!entries || entries.length === 0) {
            container.innerHTML = '<div class="empty-state">No guestbook entries found.</div>';
            return;
        }
        ...
        entries.forEach(entry => {
            ...
            if (btnApprove) {
                btnApprove.addEventListener('click', async () => {
                    await handleModeration(entry.id, true, el, btnApprove, el.querySelector('.btn-reject'));
                });
            }
            ...
            if (btnReject) {
                btnReject.addEventListener('click', async () => {
                    await handleModeration(entry.id, false, el, el.querySelector('.btn-approve'), btnReject);
                });
            }
        ...
        container.innerHTML = '<div class="empty-state error-message">Failed to load guestbook entries.</div>';
    } finally {
        hideLoading();
    }
}

async function handleModeration(id, isApproved, element, btnApprove, btnReject) {
    try {
