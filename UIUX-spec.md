...
Only one pet is active at a time, matching whichever theme (base or special) is currently selected — base themes simply show no pet. Pets are simple walking sprites (CSS/SVG-based, no heavy animation library needed), styled after the VS Code Pets extension's pacing behavior, with one small click-triggered reaction animation each. Cursor styles for these three themes are custom (crosshair, glowing dot, pixel-dot); base themes use the default cursor.
